...
export default function DashboardHomePage() {
  ...
  // Sync liveLogs with actual database scan results on initial fetch or refresh
  useEffect(() => {
    if (data.discoveryPulse && data.discoveryPulse.length > 0) {
      const initialLogs = data.discoveryPulse.map((result) => {
        let text = `Scan Ingested: No threat detected.`;
        if (result.status === 'matched') {
          text = `CRITICAL MATCH FOUND (${result.matchConfidence}% confidence) - ${result.sourceUrl}`;
        } else if (result.status === 'pending_match') {
          text = `Analyzing candidate match signature - ${result.sourceUrl}`;
        } else {
          text = `Ingested candidate: No matches found - ${result.sourceDomain || 'unknown'}`;
        }
        return {
          id: result._id,
          platform: result.platform,
          text,
          time: new Date(result.scrapedAt || result.createdAt || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })
        };
      });
  ...
  // Bind real-time listener for alerts generated by the scan/violations service via Socket.io
  useEffect(() => {
    const handleNewAlerts = (e) => {
             ...
             <div className="space-y-0.5 flex flex-col justify-end h-full p-2">
               {liveLogs.length > 0 ? (
                 ...
                 <div className="flex flex-col items-center justify-center h-full text-slate-400">
                   <Activity className="w-8 h-8 opacity-25 animate-pulse mb-2" />
                   <p className="text-[10px] font-bold uppercase tracking-widest">Listening for live scan events</p>
                 </div>
               )}
             </div>
          </div>
